...
use self::velocity::get_velocity_versions;
...
#[derive(Debug, Clone, TS, Serialize, Deserialize, PartialEq)]
#[ts(export)]
pub struct VelocityBuildVersion(i64);
...
pub enum Flavour {
    ...
    Velocity {
        build_version: Option<VelocityBuildVersion>,
    },
}
...
impl From<FlavourKind> for Flavour {
    fn from(kind: FlavourKind) -> Self {
        match kind {
            ...
            FlavourKind::Velocity => Flavour::Velocity {
                build_version: None,
            },
        }
    }
...
impl ToString for Flavour {
    fn to_string(&self) -> String {
        match self {
            ...
            Flavour::Velocity { .. } => "velocity".to_string(),
        }
    }
}
...
impl ToString for FlavourKind {
    fn to_string(&self) -> String {
        match self {
            ...
            FlavourKind::Velocity => "velocity".to_string(),
        }
    }
}
...
impl MinecraftInstance {
    pub async fn setup_manifest(flavour: &FlavourKind) -> Result<SetupManifest, Error> {
        let versions = match flavour {
            ...
            FlavourKind::Velocity => get_velocity_versions().await,
        }
    ...
    pub async fn new(
        ...
        let is_velocity = matches!(config.flavour, Flavour::Velocity { .. });
        let mut dir_create = tokio::fs::create_dir_all(&path_to_instance)
            .await
        ...
        // Step 2: Download JRE
        let (url, jre_major_version) = if is_velocity {
            ...
            get_jre_url(config.version.as_str())
                .await
